        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(squSaleAttr,index) in squSaleAttrList " :key="squSaleAttr.id" >
                <dt class="title">{{ squSaleAttr.saleAttrName  }}</dt>
                <dd changepirce="0" :class="{active:spuSaleAttrValue.isChecked==1}" 
                v-for="(spuSaleAttrValue,index) in squSaleAttr.spuSaleAttrValueList"
                 :key="spuSaleAttrValue.id"
                 @click="changeActive(spuSaleAttrValue,squSaleAttr.spuSaleAttrValueList)">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 这里不仅进行了路由的跳转，还把你购买的产品信息通过请求的形式 -->
                <!-- 通知给了服务器 服务器进行相应的存储 -->
  ...
  export default {
    ...
    methods:{
     ...
     async addShopcar(){
        ...
        this.$router.push({path:"/shopcart",query:{skuNum:this.skuNum}})
        }catch(error){
         alert (error.message)
        }
    ...
    computed:{
      ...mapGetters(['categoryView',"skuInfo","squSaleAttrList"]),
      // 给子组件的数据
      skuImageList(){
        // 如果服务器的数据没有回来，那么skuInfo这个对象是个空对象
